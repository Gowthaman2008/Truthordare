...
app.get('/api/stats', (req, res) => {
    ...
    res.json({
        ...
        activeRooms: roomManager.rooms?.size || 0
    });
});
...
io.on('connection', (socket) => {
    ...
    socket.on('createRoom', (data, callback) => {
        try {
            const { code, token, room } = roomManager.createRoom(socket.id, data.name, data.avatar, data.settings);
            socket.join(code);
    ...
    socket.on('joinRoom', (data, callback) => {
        try {
            ...
            const res = roomManager.joinRoom(code, socket.id, data.name, data.avatar, data.token);
